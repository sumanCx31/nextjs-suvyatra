...
const PaymentVerifyContent = () => {
  ...
  const verify = async (pidx: string, orderId: string) => {
    try {
      ...
      if (res.data?.data?.status === "Completed" || res.data?.status === "SUCCESS") {
        setStatus("success");
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-6 text-center">
      <Suspense 
        fallback={
          <div className="max-w-md w-full bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-xl">
             <Loader2 className="animate-spin text-slate-500 mx-auto" size={48} />
             <p className="text-slate-400 text-sm mt-4">Loading verification module...</p>
          </div>
        }
      >
